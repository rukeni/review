...
import { getTitleAboutTheHour } from "@/utils/getTitleAboutTheHour"
...
const dashboard = () => {
  ...
  const handleEditableClick:MouseEventHandler = event => {
    event.preventDefault();
    setIsEditable(true);
  }
  const handleEnterPress:KeyboardEventHandler = event => {
    if(event.key === 'Enter') {
      event.preventDefault();
      event.stopPropagation();
      setIsEditable(false);
      const currentColor = stringToColor(name);
  ...
  useEffect(() => {
      ...
      setName(storedName || '이름없음');
      if(storedName) {
        const currentColor = stringToColor(storedName);
        setBgColor(currentColor);
      } else {
        setBgColor('#FFFFFF');
      }
  ...
  const title = getTitleAboutTheHour();
  return (
    <main className="transition bg-gray-100 dark:bg-gray-800 relative h-screen overflow-hidden" style={{
      background: bgColor,
      color: color,
  ...
  <div className="flex items-start justify-between">
    ...
    <div className="flex flex-col w-full md:space-y-4">
      <div className="overflow-auto h-screen pb-24 px-4 md:px-6">
        ...
        <h1 className="difference text-4xl mt-3 mb-1 font-semibold dark:text-white" style={{ color: color}}>
          {title} 
          {isEditable ? (
            <input type="text" style={{background: bgColor, border: `1px solid ${color}`}} onKeyDown={handleEnterPress} onBlur={handleBlur} value={name} onChange={handleChange} />
          ): (
            <button onClick={handleEditableClick}>{name}님</button>
          )}
        </h1>
        <h2 className="difference text-md">
          서두르지 말되 멈추지도 마라 -스페인 속담-
        </h2>
        </div>
